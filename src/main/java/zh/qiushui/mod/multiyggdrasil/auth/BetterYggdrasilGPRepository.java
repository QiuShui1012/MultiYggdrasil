...
public class BetterYggdrasilGPRepository implements GameProfileRepository {
    private static final Logger LOGGER = LoggerFactory.getLogger(BetterYggdrasilGPRepository.class);
    ...
    private final List<URL> searchPageUrls;

    public BetterYggdrasilGPRepository(final Proxy proxy, final List<Environment> environments) {
        this.client = MinecraftClient.unauthenticated(proxy);
        searchPageUrls = Lists.transform(
            environments,
            environment -> HttpAuthenticationService.constantURL(environment.servicesHost() + "/minecraft/profile/lookup/bulk/byname"));
    }
    ...
    public void findProfilesByNames(final String[] names, final ProfileLookupCallback callback) {
        ...
        for (final List<String> request : Iterables.partition(criteria, ENTRIES_PER_PAGE)) {
            final List<String> normalizedRequest = request.stream().map(BetterYggdrasilGPRepository::normalizeName).toList();

            for (URL searchPageUrl : searchPageUrls) {
                singleFindProfilesByName(callback, page, request, normalizedRequest, searchPageUrl);
            }
        }
    }

    private void singleFindProfilesByName(
        final ProfileLookupCallback callback,
        AtomicInteger page,
        List<String> request, List<String> normalizedRequest,
        URL searchPageUrl
    ) {
        int failCount = 0;
        boolean failed;
        ...
        do {
            ...
            try {
                final ProfileSearchResultsResponse response = client.post(
                    searchPageUrl, normalizedRequest, ProfileSearchResultsResponse.class);
